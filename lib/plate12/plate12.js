/* plate12.js. Copyright 2017 Tomasz Konopka. */
...
makealive.lib.bioplate01 = function (obj, x) {
    ...
    var xargs = [
        ...
        makealive.defArg("data", "array:well:value:label", "contents of bar plot", null),
        ...
        makealive.defArg("platecolor", "color", "color of plate lines", "#000000"),
        makealive.defArg("wellcolor", "color", "colors of wells", "#dd0000")
    ];
    ...
    var getWellColor = function (d) {
        ...
        if (libtools.isColor(well.value)) {
            return well.value;
        }
        // if value is not a color, try to interpret as a shading values
        return libtools.shadeColor(x.wellcolor, 1 - well.value);
    };
    ...
    var tooltiphide = function (d) {
        var thiswell = d3.select(this);
        thiswell.attr("fill", libtools.l2(thiswell.attr("fillcolor")));
    ...
    x.data = libtools.indexBy(x.data, "well");
    svg.selectAll(".well").data(wellpositions).enter().append("circle")
            .attr("class", "well")
            .attr("fill", function (d) {
                return libtools.l2(getWellColor(d));
            })
